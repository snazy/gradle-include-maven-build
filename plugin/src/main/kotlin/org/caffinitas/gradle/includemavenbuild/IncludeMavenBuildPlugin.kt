...
@Suppress("unused")
internal open class IncludeMavenBuildPlugin
@Inject
constructor(private val objects: ObjectFactory) : Plugin<Settings> {
  ...
  private fun addMavenProject(
    ...
    val projectPath = inclBuild.groupArtifactToProjectPath.transform(groupArtifact)
    ...
    gradleProject.name = inclBuild.groupArtifactToProjectName.transform(groupArtifact)
    gradleProject.buildFileName = inclBuild.gradleBuildFileName.get()

    ext.perProjectActions[projectPath] =
      objects.newInstance(GradleProjectConfigurer::class.java, inclBuild, mavenProject, ext)
